...
Kept out of the migration file so it can be unit-tested; the migration calls it with historical
model classes via ``apps.get_model``. It only touches fields present in both the historical and
current Talk/Event models (``event``, the Talk->Room FK, and the already-required ``Room.event``),
...
def backfill_talk_events(
    *,
    ...
    Rooms are event-scoped by the time this runs (migration 0027), so a talk with a room inherits
    that room's event (always coherent). A talk with no room falls back to the newest event, when
    any exists; otherwise it is left unassigned (degenerate DB with talks but no events).
    Idempotent: only ``event IS NULL`` talks are touched.
    """
