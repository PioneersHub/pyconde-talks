...
#   1. ``DJANGO_DB_POOL=True`` (Postgres + psycopg3 only) uses Django's native connection pool, so a
...
#   2. ``DJANGO_DB_POOL=False`` (default) uses ``CONN_MAX_AGE`` to keep a single connection open
...
# OAuth2 client credentials for the validation API (all three required to enable)
EMAIL_VALIDATION_API_OAUTH2_CLIENT_ID = env("EMAIL_VALIDATION_API_OAUTH2_CLIENT_ID", default="")
EMAIL_VALIDATION_API_OAUTH2_CLIENT_SECRET = env(
    "EMAIL_VALIDATION_API_OAUTH2_CLIENT_SECRET",
    default="",
)
EMAIL_VALIDATION_API_OAUTH2_TOKEN_URL = env("EMAIL_VALIDATION_API_OAUTH2_TOKEN_URL", default="")
